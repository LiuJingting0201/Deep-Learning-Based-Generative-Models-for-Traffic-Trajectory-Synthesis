"""Train and evaluate a hybrid multi-task delta-displacement ResNet18 decoder.

The model shares a scratch ResNet18 image encoder and predicts both normalized
delta displacement trajectories and normalized start points. Absolute
trajectories are evaluated by integrating predicted deltas from the learned
start, plus an oracle-start comparison using the true start point.
"""
...
class HybridMultiHeadResNet18Decoder(nn.Module):
    """Scratch ResNet18 backbone with separate delta and start regression heads."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results_hpc/hybrid_multitask_delta_decoder"))
...
def train(args: argparse.Namespace, split_metadata: pd.DataFrame, device: torch.device) -> None:
    ...
    train_loader = make_loader(args, train_metadata, delta_scaler, start_scaler, shuffle=True, device=device)
    val_loader = make_loader(args, val_metadata, delta_scaler, start_scaler, shuffle=False, device=device)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = run_epoch(model, train_loader, delta_scaler, start_scaler, device, args, optimizer)
        compute_diag = epoch == 1 or epoch % args.metrics_every == 0
        train_diag = evaluate_loader(model, train_loader, delta_scaler, start_scaler, device, args, compute_diag)
        val_diag = evaluate_loader(model, val_loader, delta_scaler, start_scaler, device, args, compute_diag)
        row = {
            "epoch": epoch,
...
def make_loader(
    ...
    return DataLoader(
        HybridDeltaDataset(args.data_root.resolve(), metadata, delta_scaler, start_scaler),
...
def run_epoch(
    ...
    with torch.set_grad_enabled(is_train):
        for batch in loader:
            loss, _ = compute_batch_losses(model, batch, delta_scaler, start_scaler, device, args)
            if is_train:
...
def evaluate_loader(
    ...
    model.eval()
    totals = {"loss": 0.0, "loss_delta": 0.0, "loss_start": 0.0, "loss_abs": 0.0}
    total_samples = 0
    ade_sum = 0.0
    fde_sum = 0.0
    with torch.no_grad():
        for batch in loader:
            loss, parts = compute_batch_losses(model, batch, delta_scaler, start_scaler, device, args)
            batch_size = batch["image"].shape[0]
            totals["loss"] += float(loss.detach().cpu()) * batch_size
            for key in ["loss_delta", "loss_start", "loss_abs"]:
...
def compute_batch_losses(
    ...
    loss_abs = torch.nn.functional.mse_loss(pred_abs, gt_absolute)
    loss = loss_delta + args.lambda_start * loss_start + args.lambda_abs * loss_abs
    return loss, {
        "loss_delta": loss_delta,
        "loss_start": loss_start,
        "loss_abs": loss_abs,
        "integrated_errors": torch.linalg.norm(pred_abs - gt_absolute, dim=2),
    }
    ...
    config.update(
        {
            ...
            "label_normalization": "train-only mean/std for delta and start labels",
            "loss": "delta_MSE + lambda_start * start_MSE + lambda_abs * integrated_absolute_MSE",
            "integration": "absolute[0]=start; absolute[1:]=start+cumsum(delta[1:])",
...
def evaluate(
    ...
    for split in ["train", "val", "test"]:
        ...
        loader = make_loader(args, split_frame, delta_scaler, start_scaler, shuffle=False, device=device)
...
def write_chinese_report(args: argparse.Namespace, summary: dict[str, object]) -> None:
    ...
    report = f"""
Hybrid Multi-task Delta Decoder 中文报告

实验设置：
- 数据集：{args.data_root.resolve()}
- 输入：RGB image / 255.0，shape [3,224,224]。
- 模型：scratch ResNet18 shared backbone，delta head 512 -> 1024 -> 448，start head 512 -> 256 -> 2。
- 归一化：delta 与 start 都只用 train split 拟合 mean/std。
- 训练目标：loss_delta + {args.lambda_start} * loss_start + {args.lambda_abs} * loss_abs。
- 积分约定：absolute[0]=start；absolute[1:]=start+cumsum(delta[1:])。
...
- centroid error={test['centroid_error']['mean']:.4f}
...
输出文件：
- config.json
- label_delta_scaler.json, label_start_scaler.json
- train_log.csv
