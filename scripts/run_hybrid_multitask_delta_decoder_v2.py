"""Train and evaluate a V2 hybrid multi-task delta-displacement ResNet18 decoder.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results_hpc/hybrid_multitask_delta_decoder_v2"))
...
def evaluate_loader(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for key in ["loss_delta", "loss_start", "loss_centroid", "loss_abs"]:
    ...
    config.update(
        {
            ...
            "loss": "delta_MSE + lambda_start * start_MSE + lambda_centroid * centroid_MSE + lambda_abs * ADE-style absolute loss",
...
- 训练目标：loss_delta + {args.lambda_start} * loss_start + {args.lambda_centroid} * loss_centroid + {args.lambda_abs} * ADE-style loss_abs。
