"""Smoke test Week3 E2 drift-aware decomp+MTF-local ResNet18."""
...
OUTPUT_DIR = PROJECT_ROOT / "week03" / "results" / "smoke_e2_drift_aware_decomp_mtf"
...
def main() -> None:
    ...
    model = DriftAwareDecompMtfLocalResNet18DeltaDecoder(dropout=0.3, correction_anchors=16).to(device)
    model.train()
    outputs = model(image)
    expected_shapes = {
...
def write_outputs(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# E2 Drift-Aware Decomp+MTF-Local ResNet18 Smoke Report",
        "",
...
def print_summary(report: dict[str, Any]) -> None:
    print("E2 drift-aware decomp+MTF-local ResNet18 smoke test passed")
