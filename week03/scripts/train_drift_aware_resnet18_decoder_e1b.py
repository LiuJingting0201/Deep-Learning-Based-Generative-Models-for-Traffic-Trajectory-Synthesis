"""Train Week3 E1b raw ResNet18 with direct correction-target supervision."""
...
def main() -> None:
    ...
    write_week3_e1b_report(args, summary)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "week03" / "results" / "experiment_e1b_drift_aware_corrtarget_raw_resnet18_bs16",
    )
...
def make_config(
    ...
    config.update(
        {
            "experiment": "E1b raw ResNet18 low-frequency drift correction with direct correction target",
            "model": "DriftAwareResNet18DeltaDecoder",
...
def predict_split(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            corr_target_norm_t = true_abs_norm_t - pred_abs_raw_norm_t
...
def write_week3_e1b_report(args: argparse.Namespace, summary: dict[str, object]) -> None:
    output_dir = args.output_dir.resolve()
    metrics = summary["metrics_summary"]
    best = json.loads((output_dir / "best_summary.json").read_text())
    report: dict[str, object] = {
        "model": "DriftAwareResNet18DeltaDecoder",
        "experiment": "E1b direct correction-target supervision",
        "input_representation": "raw RGB [GASF,GADF,MTF] in [0,1]",
        "correction_anchors": args.correction_anchors,
        "loss_weights": {
    ...
    write_json(output_dir / "week3_e1b_report.json", report)
    write_markdown_report(output_dir / "week3_e1b_report.md", report)
    print((output_dir / "week3_e1b_report.md").read_text())
...
def write_markdown_report(path: Path, report: dict[str, object]) -> None:
    lines = [
        "# Week3 E1b Drift-Aware Raw ResNet18 Report",
        "",
        ...
        "",
        "| split | delta ADE | delta FDE | raw ADE | raw FDE | corrected ADE | corrected FDE | ADE improvement % | FDE improvement % | corr target MSE | pred/target norm ratio | raw length ratio | corrected length ratio |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
