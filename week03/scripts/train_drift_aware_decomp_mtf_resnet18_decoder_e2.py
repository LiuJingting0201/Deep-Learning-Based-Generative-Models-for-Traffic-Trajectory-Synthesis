"""Train Week3 E2 D-representation ResNet18 with drift correction."""
...
from drift_aware_decomp_mtf_resnet18_models import DriftAwareDecompMtfLocalResNet18DeltaDecoder  # noqa: E402
...
def main() -> None:
    ...
    write_week3_e2_report(args, summary)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "week03" / "results" / "experiment_e2_drift_aware_decomp_mtf_local_resnet18_bs16",
    )
    ...
    parser.add_argument("--lambda-abs", type=float, default=0.1)
    parser.add_argument("--lambda-corr-target", type=float, default=0.1)
    parser.add_argument("--lambda-smooth", type=float, default=0.001)
    parser.add_argument("--lambda-mag", type=float, default=0.00005)
...
def train(args: argparse.Namespace, split_metadata: pd.DataFrame, device: torch.device) -> None:
    ...
    model = DriftAwareDecompMtfLocalResNet18DeltaDecoder(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"val_abs={val_losses['abs_loss']:.6f} train_corr={train_losses['corr_target_loss']:.6f} "
            f"val_corr={val_losses['corr_target_loss']:.6f} best_val={best_val_loss:.6f} best_epoch={best_epoch}"
        )
...
def make_config(
    ...
    config.update(
        {
            "experiment": "E2 D-based low-frequency drift correction with direct correction target",
            "model": "DriftAwareDecompMtfLocalResNet18DeltaDecoder",
            "backbone": "torchvision.models.resnet18(weights=None)",
            "pretrained": False,
            "conv1_input_channels": 6,
            "input_representation": "[R_sym,R_res,G_antisym,G_res,B_raw,B_local]",
            ...
            "correction_target": "true_abs_norm - pred_abs_raw_norm.detach()",
            "baseline_reference": "Experiment D integrated ADE/FDE = 138.10 / 230.69",
...
def compute_losses(
    ...
    return {
        ...
        "abs_loss": abs_loss,
        "corr_target_loss": corr_target_loss,
...
def empty_loss_totals() -> dict[str, float]:
    return {
        key: 0.0
        for key in ["total_loss", "delta_loss", "abs_loss", "corr_target_loss", "smooth_loss", "mag_loss"]
    }
...
def write_train_log(path: Path, rows: list[dict[str, object]]) -> None:
    fieldnames = [
        ...
        "train_delta_loss",
        "train_abs_loss",
        "train_corr_target_loss",
        "train_smooth_loss",
        "train_mag_loss",
        "val_total_loss",
        "val_delta_loss",
        "val_abs_loss",
        "val_corr_target_loss",
        "val_smooth_loss",
        "val_mag_loss",
        "learning_rate",
        "elapsed_time",
...
def plot_training_curves(rows: list[dict[str, object]], plots_dir: Path) -> None:
    ...
    for name, columns in {
        ...
        "component_train_loss_curves.png": [
            "train_delta_loss",
            "train_abs_loss",
            "train_corr_target_loss",
            "train_smooth_loss",
...
def predict_split(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            true_abs_t = batch["absolute"].to(device, non_blocking=True)
            true_abs_norm_t = scaler_transform_tensor(absolute_scaler, true_abs_t)
            corr_target_norm_t = true_abs_norm_t - pred_abs_raw_norm_t.detach()
            ...
            corr_target_norm_np = corr_target_norm_t.cpu().numpy().astype(np.float32)
            ...
            for index, sample_id in enumerate(batch["sample_id"]):
                ...
                sample_corr_target = corr_target_norm_np[index]
                ...
                corr_target_norm = np.linalg.norm(sample_corr_target, axis=1)
                corr_smooth = float(np.mean(np.diff(sample_corr, axis=0) ** 2))
                corr_target_mse = float(np.mean((sample_corr - sample_corr_target) ** 2))
                corr_target_norm_mean = float(corr_target_norm.mean())
                rows.append(
                    {
                        "sample_id": sample_id,
                        ...
                        **corrected_metrics,
                        ...
                        "correction_target_mse": corr_target_mse,
                        "correction_target_norm_mean": corr_target_norm_mean,
                        "correction_pred_to_target_norm_ratio": float(corr_norm.mean() / corr_target_norm_mean)
                        if corr_target_norm_mean
                        else np.nan,
                    }
                )
...
def summarize_metrics(per_sample: pd.DataFrame) -> dict[str, dict[str, dict[str, float]]]:
    metrics = [
        ...
        "correction_target_mse",
        "correction_target_norm_mean",
        "correction_pred_to_target_norm_ratio",
    ]
...
def write_week3_e2_report(args: argparse.Namespace, summary: dict[str, object]) -> None:
    output_dir = args.output_dir.resolve()
    metrics = summary["metrics_summary"]
    best = json.loads((output_dir / "best_summary.json").read_text())
    report: dict[str, object] = {
        "model": "DriftAwareDecompMtfLocalResNet18DeltaDecoder",
        "experiment": "E2 D-based direct correction-target supervision",
        "input_representation": "[R_sym,R_res,G_antisym,G_res,B_raw,B_local]",
        "baseline_reference": {"experiment_d_integrated_ADE": 138.10, "experiment_d_integrated_FDE": 230.69},
        "correction_anchors": args.correction_anchors,
        "loss_weights": {
            "lambda_abs": args.lambda_abs,
            "lambda_corr_target": args.lambda_corr_target,
    ...
    for split in ["train", "val", "test"]:
        ...
        improvement_vs_d_ade = percent_improvement(138.10, corrected_ade) if split == "test" else None
        improvement_vs_d_fde = percent_improvement(230.69, corrected_fde) if split == "test" else None
        report["splits"][split] = {
            ...
            "correction_target_loss": split_metrics["correction_target_mse"]["mean"],
            "correction_target_norm_mean": split_metrics["correction_target_norm_mean"]["mean"],
            "correction_pred_to_target_norm_ratio": split_metrics["correction_pred_to_target_norm_ratio"]["mean"],
            ...
            "corrected_ADE_improvement_vs_experiment_d_percent": improvement_vs_d_ade,
            "corrected_FDE_improvement_vs_experiment_d_percent": improvement_vs_d_fde,
        }
    write_json(output_dir / "week3_e2_report.json", report)
    write_markdown_report(output_dir / "week3_e2_report.md", report)
    print((output_dir / "week3_e2_report.md").read_text())
...
def write_markdown_report(path: Path, report: dict[str, object]) -> None:
    lines = [
        "# Week3 E2 Drift-Aware Decomp+MTF-Local ResNet18 Report",
        "",
        f"- experiment: {report['experiment']}",
        f"- model: {report['model']}",
        f"- input_representation: {report['input_representation']}",
        f"- baseline_reference: {report['baseline_reference']}",
        ...
        "",
        "| split | delta ADE | delta FDE | raw ADE | raw FDE | corrected ADE | corrected FDE | raw improvement % | raw FDE improvement % | improvement vs D ADE % | improvement vs D FDE % | corr target MSE | pred/target norm ratio | raw length ratio | corrected length ratio |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    for split in ["train", "val", "test"]:
        ...
        lines.append(
            ...
            f"{format_optional_percent(row['corrected_ADE_improvement_vs_experiment_d_percent'])} | "
            f"{format_optional_percent(row['corrected_FDE_improvement_vs_experiment_d_percent'])} | "
            f"{row['correction_target_loss']:.6f} | {row['correction_pred_to_target_norm_ratio']:.6f} | "
            f"{row['raw_trajectory_length_ratio']:.6f} | {row['corrected_trajectory_length_ratio']:.6f} |"
        )
    lines.extend(
        [
            "",
            "The comparison against Experiment D uses the controlled Week3 test result of D: "
            "integrated ADE/FDE = 138.10 / 230.69. Therefore, improvement vs D is reported only for the test split.",
            "",
            "A pred/target norm ratio near 1 suggests the correction head learned the needed correction magnitude. "
            "A ratio far below 1 suggests the head is still under-correcting drift.",
        ]
    )
    path.write_text("\n".join(lines) + "\n")
